...
**Visible result:** A chat interface where students interact with an AI tutor. Responses stream token by token with LaTeX formula rendering and syntax-highlighted code blocks. The AI uses a graduated hint system, adapting both the amount of information revealed and the communication style to each student's ability. Development is split into two parts: Part A delivers text-based chat, and Part B adds file and image uploads.
...
- **Negligible cost.** Commercial embedding tokens cost a fraction of LLM inference tokens. The cost of embedding every user message is effectively zero compared to generating a response.
...
No LLM call is made. This prevents misuse and reduces unnecessary API spending.
...
**Why embeddings instead of code hashing:** The original design used SHA-256 hashing of code, which breaks on minor edits (a single whitespace change produces a completely different hash). Semantic similarity is far more robust: it captures meaning rather than exact text, works across different phrasings of the same question, and naturally extends to multimodal inputs (text and images) in Part B.
...
| Message Type | Handled By | LLM Cost |
|-------------|-----------|:--------:|
| Greetings and small talk | Embedding filter and canned response | None |
| Off-topic questions | Embedding filter and rejection message | None |
| Elaboration requests (same problem) | Elaboration anchors and hint escalation | Main LLM call only |
| Substantive follow-ups (same problem) | Q+A context similarity and hint escalation | Classification call + main LLM call |
| Genuine new questions | Full pipeline | Classification call + main LLM call |
...
- `chat_sessions`, `chat_messages`, and `daily_token_usage` database tables.
...
- Daily token usage tracking displayed as a percentage on the user profile.
...
| `module_id` | UUID | Nullable, foreign key to `learning_modules.id` (Phase 3) |
...
| Provider | Model | Input Cost (per MTok) | Output Cost (per MTok) | Implementation File |
|---------|-------|---------:|----------:|-------------------|
| Anthropic | Claude Sonnet 4.5 | $3.00 | $15.00 | `llm_anthropic.py` |
| Google | Gemini 3 Pro Preview | $2.00 | $12.00 | `llm_google.py` |
| OpenAI | GPT-5.2 | $1.75 | $14.00 | `llm_openai.py` |
...
The compression function sends the older messages to the LLM with a short summarisation prompt, requesting a 2-3 sentence summary that preserves key topics, problems discussed, and important context. The summary output is limited to 300 tokens.
...
| `/api/chat/usage` | GET | Returns today's token usage: input and output tokens used, daily limits, and the display percentage (whichever is higher). |
...
### 4.12 Markdown, Code, and LaTeX Rendering
...
### 4.13 Update Routing and Profile
...
**`Navbar.tsx`:** Enable the Chat link (previously a disabled placeholder in Phase 1).

**`ProfilePage.tsx`:** Add a daily usage progress bar showing "Daily usage: X% used". The percentage is fetched from `GET /api/chat/usage`.

### 4.14 Alembic Migration
...
- Creating the `daily_token_usage` table.
